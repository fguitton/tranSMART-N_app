...
class RWGController {
    ...
    def index = {
		...
		if (rwgSearchOperators) {
			rwgSearchOperators = rwgSearchOperators.join(searchOpDelimiter)
		}
		else {
			rwgSearchOperators = "";
		}
		...
		return [rwgSearchFilter: rwgSearchFilter, rwgSearchOperators: rwgSearchOperators, globalOperator: globalOperator, rwgSearchCategory: searchCategory, debug: params.debug];
   ...
   */
    ...
    */
    ...
    private String createSOLRNonfacetedQueryString(List queryParams ) {
	   def nonfacetedQuery=""
	   // loop through each regular query parameter
  ...
  */
   ...
   */
   ...
   def getFacetResultsForTable = {
    ...
    def getFacetResults2 = {
        ...
        def searchOperators = searchOperatorsString?.split(searchOpDelimiter)
        ...
        if (!params.page || params.page.equals('RWG'))
        {
            redirect(action: "getFacetResults", params: params);
            return;
        }
   ...
   * @return JSON object containing facet counts
   */
  private  def getInitialFacetResults = {List categoriesList  ->
...
//	   imagedata = Base64.decodeBase64(imagedata)
  ...
  def browseDataTypesMultiSelect = {
	  ...
	  */
	  ...
	  def dataTypes = ["GWAS":"GWAS","EQTL":"eQTL","Metabolic GWAS":"Metabolic GWAS","Gene Expression":"Gene Expression"]
