...
class SolrFacetService {
			...
			*/
			if (!categoryName.equals("text")) {
				...
				if (page.equals('RWG')) {
                    ...
                    categoryResultIds += getNodesByAccession(getAccessions2(xml))
                    /*
                    if (categoryName.equals("GENE")) {
                        searchLog += "Getting analyses for gene categories..."
	...
	def getAnalysesForGenes(termList, operator, convertToNodes) {
        ...
        return analysisResults;    //TODO: SEB - shortcircuiting this method until we add Annotation Manager back in
   ...
   def getAccessions2(xml) {
        ...
        for (node in resultNodes) {
            ...
            def arrNode = node.arr

            if (!arrNode) continue

            def accessionNodes = node.arr.findAll{it.@name == 'ACCESSION'}

            for (accessionNode in accessionNodes)
            {
                def accessionId = accessionNode.str.text()
                accessions.push(accessionId)
            }
        }
  ...
  */
  def executeSOLRFacetedQuery = {solrRequestUrl, solrQueryParams, returnAnalysisIds ->
	  ...
	  dataWriter.write("&fl=ANALYSIS_ID,ACCESSION")
	  dataWriter.flush()
	  dataWriter.close()
